...
public sealed class GitHubService(IHttpClientFactory httpClientFactory, ILogger<GitHubService> logger) 
{
        ...
        CancellationToken cancellationToken = default) 
    {
        ...
        {
            logger.LogWarning($"Failed to get GitHub user profile. Status code: {response.StatusCode}");
            return null;
        }
        ...
        CancellationToken cancellationToken = default) 
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            logger.LogWarning($"Failed to get GitHub user events. Status code: {response.StatusCode}");
            return null;
        }
